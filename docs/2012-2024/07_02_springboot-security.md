...
## 1.2 Disable security in SB App
```java
...
## 6. Rate limiting per IP/client `pending`
...
## 7. encryption
- HTTPS, TLS handshake, X.509 cert, cert rotation
- enforce https over http using header: `Strict-Transport-Security`
- k8s | ngInx 👈🏻
- **openssl** tool

## 8. ✔️Secrets Management & Config
- AWS Secrets Manager
- AWS SSM paramStore

## 9. Resiliency
- ensure security layers do not cause unacceptable latency;
- demonstrate circuit breaker for **Okta auth server downtime** `pending`
